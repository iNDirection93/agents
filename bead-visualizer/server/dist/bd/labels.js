...
export class LabelReader {
     ...
     */
    agentOf(bead) {
        if (bead.assignee && this.agents.has(bead.assignee))
            return bead.assignee;
        const present = bead.labels.filter((l) => this.agents.has(l));
        if (present.length === 0)
            return bead.assignee ?? null;
        const origins = new Set(bead.labels
     ...
     */
    isGate(bead) {
        return bead.labels.some((l) => this.cfg.gateLabels.includes(l) || this.cfg.gateLabelPrefixes.some((p) => l.startsWith(p)));
    }
...
export function groupBySession(beads, reader) {
    const out = new Map();
