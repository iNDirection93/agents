 ...
 * of an empty one.
 */
export function derivePreview(sessionId, sessionBeads, openBeadsEverywhere, mode) {
 ...
 */
export function buildReferenceIndex(closedIds, openBeads) {
    const index = new Map();
    if (closedIds.length === 0)
        return index;
    const patterns = closedIds.map((id) => [id, new RegExp(`(^|[^A-Za-z0-9_-])${escapeRe(id)}([^A-Za-z0-9_-]|$)`)]);
    for (const open of openBeads) {
        if (!open.referenceText)
            continue;
...
export function parseDryRunJson(value) {
    ...
    for (const entry of asArray(kept)) {
        ...
        if (refs.length > 0 || /referen|cite/.test(reason))
            out.keptReferenced.set(id, refs);
        else
            out.keptOpen.push(id);
    }
 ...
 */
export function parseDryRunText(text, knownIds) {
    const out = emptyDryRun();
    if (!text.trim() || knownIds.length === 0)
        return { ...out, estimatedBytes: parseSize(text) };
    const patterns = knownIds.map((id) => [id, new RegExp(`(^|[^A-Za-z0-9_-])${escapeRe(id)}([^A-Za-z0-9_-]|$)`)]);
    let section = null;
...
function classifyHeading(line) {
    ...
    if (!l || /^[-*\d]/.test(l))
        return null;
    if (SECTION_DELETE.test(l))
        return 'delete';
    if (SECTION_KEEP_REF.test(l))
        return 'keep-ref';
    if (SECTION_KEEP.test(l))
        return SECTION_KEEP_OPEN.test(l) ? 'keep-open' : 'keep';
    return null;
}
function classifyInline(line) {
    if (SECTION_KEEP_REF.test(line))
        return 'keep-ref';
    if (SECTION_KEEP.test(line) && SECTION_KEEP_OPEN.test(line))
        return 'keep-open';
    if (SECTION_KEEP.test(line))
        return 'keep-open';
    if (SECTION_DELETE.test(line))
        return 'delete';
    return null;
}
const SIZE_UNITS = {
 ...
 */
export function mergePreview(derived, parsed, beadsById) {
