...
export async function registerAvatars(app, dir) {
    ensureFallback(dir);
...
export function readManifest(dir) {
    ...
    const empty = { version: 1, fallback: GENERIC_FALLBACK, agents: {}, warnings: [] };
    if (!existsSync(path)) {
    ...
    try {
        ...
        return { ...empty, warnings: [`${path} is not valid JSON: ${e.message}`] };
    }
    ...
    const fallback = typeof raw.fallback === 'string' && isSafeRelativePath(raw.fallback) && existsSync(join(dir, raw.fallback))
        ? raw.fallback
        : GENERIC_FALLBACK;
