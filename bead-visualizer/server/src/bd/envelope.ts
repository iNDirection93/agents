...
const ENVELOPE_DATA_KEYS = ['data', 'result', 'results', 'issues', 'beads', 'items', 'rows'] as const;

export function parseJsonStdout(stdout: string, what: string): unknown {
  ...
  throw new AppError(
    ...
    'Run the command by hand with BD_JSON_ENVELOPE=1 and compare the output shape; the parsers in server/src/bd/envelope.ts may need a new case.',
    trimmed.slice(0, 400),
  );
}
...
function sliceFromFirstJson(text: string): string | null {
 ...
 */
export function unwrapEnvelope(value: unknown, what: string): unknown {
...
export function parseRecords(stdout: string, what: string): Record<string, unknown>[] {
...
export function pick(rec: Record<string, unknown>, ...names: string[]): unknown {
  for (const name of names) {
    ...
    if (rec[snake] !== undefined && rec[snake] !== null) return rec[snake];
  }
  return undefined;
}

export function pickString(rec: Record<string, unknown>, ...names: string[]): string | null {
...
export function pickNumber(rec: Record<string, unknown>, ...names: string[]): number | null {
  const v = pick(rec, ...names);
...
export function pickStringArray(rec: Record<string, unknown>, ...names: string[]): string[] {
  const v = pick(rec, ...names);
  if (Array.isArray(v)) {
    return v
