...
async function main(): Promise<void> {
  ...
  app.log.info(
    { repoRoot: cfg.repoRoot, avatars: cfg.avatarsDir, source: backend.source, sessionLabelPattern: cfg.sessionLabelPattern },
    `bead-visualizer sidecar on http://127.0.0.1:${cfg.port}`,
  );
...
main().catch((e: unknown) => {
  console.error(`[bead-visualizer] ${(e as Error).message}`);
  process.exit(1);
});
